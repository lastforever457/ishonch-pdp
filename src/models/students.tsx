...
export const useUpdateStudent = () => {
  ...
  const { t } = useTranslation();
  const data = useMutation({
    mutationFn: async (data: { id: string | number; data: Record<string, any> }) => {
...
export const useConnectStudentToGroup = () => {
  const queryClient = useQueryClient();
  const data = useMutation({
    mutationFn: async (data: { studentId: string | number; groupId: string }) => {
      const res = await api.post(`/group/addNewReader/${data.studentId}/${data.groupId}`);
      await queryClient.invalidateQueries({
...
export const useDisconnectStudentFromGroup = () => {
  ...
  const data = useMutation({
    mutationFn: async (data: { studentId: string | number; groupId: string }) => {
      const res = await api.post(`/group/remove/${data.studentId}/${data.groupId}`);
      await queryClient.invalidateQueries({
        queryKey: ['group-profile'],
      });
      return res.data;
