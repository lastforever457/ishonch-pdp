...
const Employees = () => {
  ...
  const { mutate: mutateCreateUser, isPending: isCreatePending } =
    useCreateUser();
  const { mutate: mutateUpdateUser, isPending: isUpdatePending } =
    useUpdateUser();
  const { mutate: mutateDeleteUser, isPending: isDeletePending } =
    useDeleteUser();
  const {
    data: users,
      ...
      },
      {
        ...
        render: (text: string) => (
          <Tooltip placement="topLeft" title={text}>
            {t(`employees.${text.toString().toLowerCase()}`)}
          </Tooltip>
        ),
      },
    ],
  ...
  if (isLoadingUsers || isCreatePending || isUpdatePending || isDeletePending) {
    return <CustomLoader />;
  }
