...
const MySegmented = ({
  ...
  return (
    <div
      ...
      className="flex bg-white mt-3 md:mt-0 p-1 rounded-2xl text-sm md:text-xl select-none"
    >
      {segmentedValues.map((item: Record<string, any>) => (
        <div
          key={item.key}
          onClick={() => push({ query: { ...query, [queryName]: item.key } })}
          className={`p-2 md:px-6 md:py-3 cursor-pointer segmented-item rounded-xl flex justify-center font-bold items-center  ${
